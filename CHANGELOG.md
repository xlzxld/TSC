...
| ID | 修复 | 实测取证 |
|---|---|---|
| B-01 | `structure_guard.py` 的 `read_source` 修复标准输入二次读取导致的坏代码假绿放行 | 修复前标准输入传入失衡代码（如 `def foo(`）因二次读 stdin 得到空串而以退出码 0 假绿；修复后以退出码 1 拦截并报告具体未闭合括号及修复建议 |
| B-02 | `skills/tsc/templates/project.example.py` 移除推荐动态拼接 `sys.executable` 的过时注释陷阱，改为使用中立通用命令并说明 runtime 自动回退 | 修复前若按旧示例拼接 `sys.executable`，新项目在 `tsc check-config` 时会因静态比对差异 100% 报错（退出码 1）；现明确同源要求与 PATH 缺失自动回退机制 |
| B-03 | `README.md` 纠正克隆与安装目录结构说明，明确软链接与目录直拷的正确路径 | 修复前指引直接将仓库克隆至宿主技能目录导致 `skills/tsc/SKILL.md` 产生二级嵌套、宿主无法识别；修复后指引软链或直拷 `skills/tsc` |
| B-04 | `skills/tsc/commands/tsc.md` 纠正体检引用路径，去除遗留的二级目录前缀 | 路径由 `skills/tsc/references/AUDIT-SPEC.md` 改为 `<技能根>/references/AUDIT-SPEC.md`，与安装后的技能目录结构保持一致 |
| B-05 | `structure_guard.py` 增加 `L2_BY_LANG` 兜底，解决无后缀可执行脚本与标准输入跳过 L2 语法检查的问题 | 修复前无后缀脚本或管道输入即使指定了语言也因缺少后缀名被跳过 L2 检查；修复后按语言 profile 正确调度对应语法检查器（如 Python `ast.parse`）并拦截语法错误 |
